...
void UIEngine::Initialise(int & argc, char** argv, ImageSourceEngine *imageSource, IMUSourceEngine *imuSource, InputSource::ExternalTrackerEngine *ExternalTracker, ITMMainEngine *mainEngine, const char *outFolder, ITMLibSettings::DeviceType deviceType, bool use_ros, bool dynamic_objects)
{
	...
	//use getRGBImageSize. Assume those different types of image has the same row and column number
	inputMaskImage = new ITMUCharImage(imageSource->getRGBImageSize(), true, allocateGPU);
...
void UIEngine::ProcessFrame()
{
	...
	if(dynamic_objects_){
		imageSource->getImages(inputRGBImage, inputRawDepthImage, inputMaskImage);
		/*
		printf("check mask image \n");
		auto data_test = inputMaskImage->GetData(MEMORYDEVICE_CPU);
		int tmp_counter = 0;
		for(int i = 0; i<inputMaskImage->noDims.x; i++){
			for(int j = 0; j<inputMaskImage->noDims.y; j++){
				if(data_test[j*inputMaskImage->noDims.x+i] == 255) tmp_counter++;
			}
		}

		printf("pixel number in the mask........ %d \n", tmp_counter);
		exit(0);
		*/
	}
	else{
	...
	if(dynamic_objects_ && inputMaskImage == NULL){
		printf("wired, we should have mask........................ \n");
	}
	...
	mainEngine->ProcessFrame(inputRGBImage, inputRawDepthImage, inputIMUMeasurement, inputMaskImage);

	//original is the following. However, since the default inputIMUMeasurement and inputMaskImage is NULL, we dont need to make decision here. We can decide if we use imu or mask by checking if they are NULL
