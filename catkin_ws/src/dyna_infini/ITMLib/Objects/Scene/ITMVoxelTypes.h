...
/** \brief
    Stores the information of a single voxel in the volume
	I use ITMVoxel_s_rgb voxel in default
*/
struct ITMVoxel_s_rgb
{
	...
	/*record if has been updated before because of dynamics objects*/
	bool dyna_updated_;
	/*if update for the first time. The first update use will have a large sdf difference normally, we should not assume it is dynamic*/
	bool ini_;
	/*prev sdf, may recover*/
	short prev_sdf_;
	/*prev weight*/
	uchar prev_w_depth_;
	/*if initialized in the mask*/
	bool ini_in_mask_;
