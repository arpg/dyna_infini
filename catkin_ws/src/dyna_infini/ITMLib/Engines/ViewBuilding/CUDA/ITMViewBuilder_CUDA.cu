...
void ITMViewBuilder_CUDA::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, bool useBilateralFilter, ITMUCharImage *maskImage, bool modelSensorNoise, bool storePreviousImage)
{
	if (*view_ptr == NULL)
	{
		*view_ptr = new ITMView(calib, rgbImage->noDims, rawDepthImage->noDims, true);
		...
		//if (this->charImage != NULL) delete this->charImage;
		//this->charImage = new ITMUCharImage(rawDepthImage->noDims, true, true);
	...
	if(maskImage != NULL)
		view->mask_->SetFrom(maskImage, MemoryBlock<uchar>::CPU_TO_CUDA);
	else
	    view->mask_ = NULL;
...
void ITMViewBuilder_CUDA::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *depthImage, bool useBilateralFilter, ITMIMUMeasurement *imuMeasurement, ITMUCharImage *maskImage, bool modelSensorNoise, bool storePreviousImage)
{
	if (*view_ptr == NULL) 
	{
	...
	this->UpdateView(view_ptr, rgbImage, depthImage, useBilateralFilter, maskImage, modelSensorNoise, storePreviousImage);
}
